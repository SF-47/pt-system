...
export default function PlanCard({ id, name, description, count, kind }: PlanCardProps) {
  ...
  if (kind === "workout") {
    return (
      <article className="flex min-h-[220px] min-w-0 flex-col justify-between rounded-xl border border-border bg-surface p-5 transition-colors hover:border-primary">
        <div>
          <div className="flex items-start justify-between gap-3">
            <h3 className="min-w-0 flex-1 text-lg leading-snug font-semibold tracking-tight text-foreground wrap-anywhere">
              {name}
            </h3>
          ...
          <p
            className="mt-2 mb-5 min-h-[63px] line-clamp-3 text-sm leading-[21px] text-muted wrap-anywhere"
            title={description}
          >
            {description}
          </p>
        ...
        <Link
          ...
          className="mt-auto inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-md bg-primary-soft px-4 text-sm font-semibold text-primary-hover transition-colors hover:bg-primary hover:text-white dark:text-foreground"
        >
  ...
  return (
    <article className="flex min-h-[220px] min-w-0 flex-col rounded-xl border border-border bg-surface p-5 transition-colors hover:border-primary">
...
export function PlanCardSkeleton({ kind = "workout" }: { kind?: "workout" | "meal" }) {
  if (kind === "workout") {
    return (
      <div aria-hidden="true" className="flex min-h-[220px] animate-pulse flex-col justify-between rounded-xl border border-border bg-surface p-5">
        <div className="flex justify-between items-start gap-3">
          <div className="h-6 w-1/2 rounded bg-border/60" />
          <div className="h-6 w-24 rounded-md bg-primary-soft" />
        </div>
        <div className="mt-4 mb-5 min-h-[63px] space-y-2">
          <div className="h-4 w-full rounded bg-border/40" />
          <div className="h-4 w-2/3 rounded bg-border/40" />
        </div>
        <div className="h-11 w-full rounded-md bg-primary-soft" />
      </div>
    );
  }

  return (
    <div aria-hidden="true" className="flex min-h-[220px] animate-pulse flex-col rounded-xl border border-border bg-surface p-5">
