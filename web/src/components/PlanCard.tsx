...
export default function PlanCard({ id, name, description, count, kind }: PlanCardProps) {
  ...
  return (
    <article className="flex min-h-[340px] min-w-0 flex-col overflow-hidden rounded-lg border border-border bg-surface transition-colors hover:border-primary">
      <div
        className="flex aspect-[16/7] w-full flex-col items-center justify-center gap-2 border-b border-border bg-primary-soft text-muted"
        aria-label="No meal plan image available"
        role="img"
      >
...
export function PlanCardSkeleton({ kind = "workout" }: { kind?: "workout" | "meal" }) {
  ...
  return (
    <div aria-hidden="true" className="flex min-h-[340px] animate-pulse flex-col overflow-hidden rounded-lg border border-border bg-surface">
      <div className="aspect-[16/7] w-full border-b border-border bg-primary-soft" />
      <div className="flex flex-1 flex-col p-5">
